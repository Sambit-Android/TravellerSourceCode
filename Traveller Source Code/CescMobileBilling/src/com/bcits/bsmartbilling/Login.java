...
import Utils.Logger;
import amr.usb.utils.UsbRecieverService;
...
public class Login extends Activity {
	...
	//public static String server_ip = "122.166.217.53:9090" ;// LIVE and MAINTEST SERVER 2.84 ;
	
	public static String server_ip = "1.23.144.187:9090" ;// live jboss 10.233;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		startService(new Intent(Login.this,UsbRecieverService.class)); // START USB SERVICE FROM THE LIB
	...
	protected void onStart() {
	    ...
	    /*// for demo purpuse
	    alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(), 3*10000, pendingIntent);
		*/
		...
		alarmManagerliveStatus.setRepeating(AlarmManager.RTC_WAKEUP, liveStatusCalendar.getTimeInMillis(), 3*10*1000, pendingStatusIntent);*/
			...
			public void onClick(View v) {
				...
				else if(strPaswrd.equals(strSiteCode) )
				{
					...
					if(haveNetworkConnection()){
						...
						.setTitle("Info")
						.setMessage( 
								"Internet Connection is Not Working. Signing in using local DB")
			...
			if(MasterLibraryFunction.IsNullOrEmpty(ErrorClass.getErrorMessages()))
			{
				...
				if(ErrorClass.getErrorMessages().contains("timed out"))
				{
					ErrorClass.errorMessage = "";
						...
						.setTitle("Info")
						.setMessage( 
								"Connection timed out.Server might be shut down.Trying Signing in using local DB")
						.setPositiveButton("OK", new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int which) {
								loginOffline();
				...
				}else if(ErrorClass.getErrorMessages().contains("java.net.SocketTimeoutException"))
				{
					...
					.setTitle("Info")
					.setMessage( 
							"Socket timed out. Signing in using local DB")
					.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
								int which) {
							loginOffline();
				...
				}else if(ErrorClass.getErrorMessages().contains("Connection refused") || ErrorClass.getErrorMessages().contains("refused") ||  ErrorClass.getErrorMessages().contains("Network unreachable"))
				{
					...
					.setTitle("Info")
					.setMessage( 
							"Network Unreachable. Signing in using local DB")
					.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
								int which) {
							loginOffline();
					...
					if (jaxrsmessage.contains("<html>")) {
						...
						if (datafromserver[0].equals("0")) {
							jaxrsmessage = "";
							 new AlertDialog.Builder(Login.this)
									.setTitle("Error")
									.setMessage("Sorry, the SiteCode or MrCode is incorrect.Please check and try again later.")
									.setPositiveButton("OK", null).show();
