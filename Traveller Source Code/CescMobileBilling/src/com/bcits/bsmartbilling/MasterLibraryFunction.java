...
public class MasterLibraryFunction extends Application  {

	public Context context ;
	public static Logger masterLogger ;


	MasterLibraryFunction(){

	}

	MasterLibraryFunction(Context context) {
		this.context = context;
	}

	MasterLibraryFunction(Context context, String tCode){
		this.context = context;
		...
		{
			UtilMaster.Lfcslab1 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCSLAB1));
			UtilMaster.Lfcslab2 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCSLAB2));
			UtilMaster.Lfcslab3 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCSLAB3));
			UtilMaster.Lfcslab4 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCSLAB4));

			UtilMaster.Lfcrate1 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCRATE1));
			UtilMaster.Lfcrate2 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCRATE2));
			UtilMaster.Lfcrate3 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCRATE3));
			UtilMaster.Lfcrate4 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCRATE4));

			UtilMaster.Lecslab1 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECSLAB1));
			UtilMaster.Lecslab2 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECSLAB2));
			UtilMaster.Lecslab3 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECSLAB3));
			UtilMaster.Lecslab4 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECSLAB4));
			UtilMaster.Lecslab5 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECSLAB5));
			UtilMaster.Lecslab6 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECSLAB6));

			UtilMaster.Lecrate1 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECRATE1));
			UtilMaster.Lecrate2 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECRATE2));
			UtilMaster.Lecrate3 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECRATE3));
			UtilMaster.Lecrate4 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECRATE4));
			UtilMaster.Lecrate5 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECRATE5));
			UtilMaster.Lecrate6 = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECRATE6));
			UtilMaster.Ltax_rate = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_TAX));


			UtilMaster.LFcReduction = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCREDUCTION)); 
			UtilMaster.LEcReduction = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECREDUCTION));
			UtilMaster. LFcMax = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_FCMAX));
			UtilMaster. LEcMax = cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_ECMAX));


			UtilMaster.mtariffdesc =  cursor_next.getString(cursor_next.getColumnIndex(DBTariffHandler.KEY_TARIFFDESC));
	...
	public boolean setECTariffRates(Context mobContext , int quarter , String tCode)
	{
	...
	public int calculate() throws Exception
	{
		...
		try{
			...
			float masterInterest =Float.parseFloat(UtilMaster.minterest.trim());  
			float masterFc = Float.parseFloat(UtilMaster.mfixed_ges.trim());
			float interestondeposit = Float.parseFloat(UtilMaster.minterestondeposit.trim());
			float PFPenalty  = 0;
			float excessLoadPenalty=0;
			UtilMaster.setMprevious_billeddate(getPrvDate(-(int)(bm*30) ,  new SimpleDateFormat("MM/dd/yyyy").parse(UtilMaster.mreading_date)));
			float masterPF =  Float.parseFloat(UtilMaster.mmaster_Pf_reading.trim());
			float 	sanctionedLoadKW= 0;
			try {
				sanctionedLoadKW = 	Float.parseFloat(UtilMaster.mconnected_load.trim());
			} catch (Exception e) {
				sanctionedLoadKW = 	0;
			}
			...
			UtilMaster.mdivision = getDivisionName(Integer.parseInt(UtilMaster. mGLogin_SiteCode.trim()));
			...
			try {
				ec = (float)doubRound ( getECForAllQuarters(this.context, units, bm, UtilMaster.mtariff.trim()) , 2);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			// This is WRT the FPPCA dated October 2015 to December 2015
			// closed the FPPCA dated JAN2016


			/*if(bm <= 3){
				//others = (float) (units * 0.09);
				//others = (float) 0;
				
				others = (float) (units * 0.02);
				

			}else{
				//others = (float) (((units/bm) * 3)* 0.09);
				//others = (float) 0;
				
				others = (float) (((units/bm) * 3)* 0.02);

			}*/

			// dated 07/04/2016 fppca for flat rates
			
			if(UtilMaster.mbillmonth.trim().equals("201606")){
				others = (float) (units * 0.02);
			}
			else{
				
				others = (float) (units * 0.01);
			}
			...
			// tax =   (float)doubRound (Float.parseFloat(UtilMaster.Ltax_rate.trim()) * units,2);	
			tax =   (float)doubRound ( calTax(ec, (float) 0.06, Integer.parseInt(UtilMaster.mtariff.trim()) ,units , bm , others),2);

			credite_rebate = (float)doubRound (getCredite_rebate(ec, fc, tax , Integer.parseInt(UtilMaster.mtariff.trim()) , units , bm ),2);



			mtrRent = (float)doubRound (Float.parseFloat( UtilMaster. mmeterrent)*bm,2);	

			/*BY SHIVANAND*/

			if(UtilMaster.mtrivector.trim().equalsIgnoreCase("1") ||(! UtilMaster.mpf_reading.trim().equalsIgnoreCase("0.0"))){
				PFPenalty = (float)ltPFPenalty(UtilMaster.mpf_reading, units, masterPF);
				excessLoadPenalty =excessLoadPenalty(UtilMaster.mbmd_reading, sanctionedLoadKW, Float.parseFloat(UtilMaster.getMmeter_constant().trim()), UtilMaster.mtariff.trim()) ;
			}
			...
			System.out.println("mtrRent :"+mtrRent);
			System.out.println("arrears :"+arrears);
			System.out.println("others :"+others);
			System.out.println("masterInterest :"+masterInterest);
			System.out.println("interestondeposit :"+interestondeposit);
			
			System.out.println("credite_rebate :"+credite_rebate);
			System.out.println("dlAdj :"+dlAdj);
			System.out.println("PFPenalty :"+PFPenalty);
			...
			if ((UtilMaster.mtariff.trim() .equalsIgnoreCase( "2")|| UtilMaster.mtariff.trim() .equalsIgnoreCase( "1") ) && ((units / bm) <= 18) ) 
			{
				
				total =  mtrRent  + arrears + masterInterest +interestondeposit - (credite_rebate + dlAdj) +PFPenalty + excessLoadPenalty;
				
			}else{
				
				total = ec+ fc+tax+mtrRent  +arrears+others+ masterInterest +interestondeposit - (credite_rebate + dlAdj) +PFPenalty + excessLoadPenalty ;
			}
	...
	public float getECForAllQuarters(Context mobContext , float nou , float bm , String tariffCode)
	{
		float totalEc = 0 ;
		String[] tdates = null ;
		Date mbDate = null ;
		...
		try
		{
			...
			mbDate = getMobileDate(new SimpleDateFormat("MM/dd/yyyy").parse(UtilMaster.mreading_date)) ;
			...
			// by shivanand 
			    ...
			    cal.setTime(new SimpleDateFormat("yyyy-MM-dd").parse(tdates[0].trim()));
			    cal.add(Calendar.DAY_OF_MONTH, -1);
			    Date tariffDate=  cal.getTime();
			...
			difBM = round1(dateDiff("m",
					new SimpleDateFormat("dd/MM/yyyy").parse(UtilMaster.mprevious_billeddate
							.trim()), tariffDate),2);
			
			
			
			float baserateForLt1 = 0.0f;
			
			
			if(difBM > 0){
				
				
				if(setECTariffRates(mobContext, 1, tariffCode)){
					
					float newBM = bm;
					
					float oldconsumtion = (nou12 * difBM);
					
					baserateForLt1 = 5.26f;
					totalEc = calEC(oldconsumtion, difBM, baserateForLt1);
					...
					if(setECTariffRates(mobContext, 0, tariffCode)){
						
						
						
						float newConsumtion = (nou12 * newBM);
						
						baserateForLt1 = 5.67f;
						totalEc = totalEc + calEC(newConsumtion, newBM, baserateForLt1);
				...
				if(setECTariffRates(mobContext, 0, tariffCode)){
					...
					float newConsumtion = (nou12 * bm);
					
					baserateForLt1 = 5.67f;
					totalEc = calEC(newConsumtion, bm, baserateForLt1 );
			...
			/* date1 :Sun Jan 18 00:00:00 IST 2015
			 03-30 13:17:53.317: I/System.out(618): date2 :Tue Mar 31 00:00:00 IST 2015
			*/
		...
		/*	for (int cnt = 0; cnt < tdates.length; cnt++) 
			{
				difBM = dateDiff("m",new SimpleDateFormat("yyyy-MM-dd").parse(tdates[cnt].trim()), getMobileDate(new SimpleDateFormat("MM/dd/yyyy").parse(UtilMaster.mreading_date)));
				if (difBM >= bm) 
				{
					if (setECTariffRates(mobContext, cnt, tariffCode)) 
					{
						...
						if(cnt > 0){
							Date test11 = new SimpleDateFormat("yyyy-MM-dd").parse(tdates[cnt-1].trim()) ;
							if(mbDate.equals(test11)){
								Calendar calendartest = Calendar.getInstance();
								calendartest.setTime(mbDate);
								calendartest.add(Calendar.DATE, (-1));
								mbDate =  new SimpleDateFormat("dd/MM/yyyy").parse(new SimpleDateFormat("dd/MM/yyyy").format(calendartest.getTime()));
							}
						}
						chBM =	round1(dateDiff("m",new SimpleDateFormat("dd/MM/yyyy").parse(pvdate1) , mbDate) , 2 );
					...
					{
						pvdate1 =  new SimpleDateFormat("dd/MM/yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(tdates[cnt].trim())) ;
						*//*****dda******//*
					} else 
					{
						pvdate1 = UtilMaster.mprevious_billeddate;
					...
					mbDate = new SimpleDateFormat("yyyy-MM-dd").parse(tdates[cnt].trim()) ;
				}
			}*/
		}catch(Exception e){
			e.printStackTrace();
	...
	public float calcFCSlabWise (float consumption,float bm)
	{
		...
		if (UtilMaster.mtariff.trim() .equalsIgnoreCase( "2")|| UtilMaster.mtariff.trim() .equalsIgnoreCase( "1")) 
		{
			...
			else if( (consumption > fcSlab1 && consumption <= (fcSlab1+fcSlab2)) || (fcSlab2 == 0) )
				fc = fcRateTwo;

			else if( (consumption > (fcSlab1+fcSlab2) && consumption <= fcSlab1+fcSlab2+fcSlab3) || (fcSlab3== 0) )
				fc = fcRateThree;

			else if( (consumption >  (fcSlab1+fcSlab2+fcSlab3) && consumption <= fcSlab1+fcSlab2+fcSlab3+fcSlab4) || (fcSlab4== 0) )
				fc = fcRateFour;
	 ...
	 * @return EC
	 */
	public float calEC (float consumption,float bm, float baseRateForLt1)
	{ 
		float ec = 0 ;
		...
		// ----------------For BJ/KJ ------------/
				...
				if(UtilMaster.getMpresentmeterstatus().trim().equals("2")){
					...
					/*ec = ( (float)(untiPerMonth * (float) 5.11) < (float)30  ) ? (30) : ( (float) ((untiPerMonth * (float) 5.11) * bm)) ;*/
					
					ec = ((float) (untiPerMonth * baseRateForLt1) < (float) 30) ? (30 * bm)
							: ((float) (untiPerMonth * baseRateForLt1 * bm));
			...
			if (UtilMaster.getMpresentmeterstatus().trim().equals("2")	&& (UtilMaster.mtariff.trim().equalsIgnoreCase("1")|| UtilMaster.mtariff.trim().equalsIgnoreCase("2")))
				ec = 0;
			...
			else if( (consumption > ecSlab1 && consumption <= (ecSlab1+ecSlab2)) || (ecSlab2 == 0) )
			{
				ec = (ecSlab1 * ecRateOne) + ((consumption - ecSlab1) * ecRateTwo);
			}
			else if( (consumption > (ecSlab1+ecSlab2) && consumption <= ecSlab1+ecSlab2+ecSlab3) || (ecSlab3== 0) )
			{
				ec = (ecSlab1 * ecRateOne) + (ecSlab2 * ecRateTwo) + ((consumption - ecSlab1-ecSlab2) * ecRateThree);
			}
			else if( (consumption >  (ecSlab1+ecSlab2+ecSlab3) && consumption <= ecSlab1+ecSlab2+ecSlab3+ecSlab4) || (ecSlab4== 0) )
			{
				ec = (ecSlab1 * ecRateOne) + (ecSlab2* ecRateTwo) +(ecSlab3 * ecRateThree)+ ((consumption - ecSlab1-ecSlab2-ecSlab3) * ecRateFour);
			}
			else if( (consumption > ( ecSlab1+ecSlab2+ecSlab3+ecSlab4 ) && consumption <= ecSlab1+ecSlab2+ecSlab3+ecSlab4+ecSlab5 ) || (ecSlab5== 0) )
			{
				ec = (ecSlab1 * ecRateOne) + (ecSlab2 * ecRateTwo) + (ecSlab3 * ecRateThree)+(ecSlab4 * ecRateFour)+((consumption -ecSlab1-ecSlab2-ecSlab3-ecSlab4 ) * ecRateFour);
			}
			else if( (consumption > (ecSlab1+ecSlab2+ecSlab3+ecSlab4+ecSlab5 )&& consumption <=  ecSlab1+ecSlab2+ecSlab3+ecSlab4+ecSlab5+ecSlab6) || (ecSlab6== 0) )
			{
				ec = (ecSlab1 * ecRateOne) + (ecSlab2 * ecRateTwo) + (ecSlab3 * ecRateThree)+(ecSlab4 * ecRateFour)+(ecSlab5 * ecRateFive)+((consumption -ecSlab1-ecSlab2-ecSlab3-ecSlab4-ecSlab5) * ecRateSix);
			}
			else if( consumption > (ecSlab1+ecSlab2+ecSlab3+ecSlab4+ecSlab5+ecSlab5+ecSlab6 ))
				ec = (ecSlab1 * ecRateOne) + (ecSlab2 * ecRateTwo) + (ecSlab3 * ecRateThree)+(ecSlab4 * ecRateFour)+(ecSlab5 * ecRateFive)+(ecSlab6 * ecRateSix)+((consumption -ecSlab1-ecSlab2-ecSlab3-ecSlab4-ecSlab5-ecSlab6) * ecRateSix);
	 ...
	 * @return Total Rebate 
	 */
	public float getCredite_rebate(float ec, float fc, float tax, int Ltariff_code_i,float consumption, float bm) 
	{
		float credite_rebate = 0 ;
		float localtempEC = 0 ;
		...
		else if (Ltariff_code_i == 54 || Ltariff_code_i == 544 	|| Ltariff_code_i == 504 || Ltariff_code_i == 204)
		{
			//1st case free Lighting FL case for LT2A I : TARIF CODE : Ltariff_code_i == 54 || Ltariff_code_i == 544 || Ltariff_code_i == 504  
			//2nd case free Lighting FL case for LT2A II : TARIF CODE :  Ltariff_code_i == 204 
			//3rd case free Lighting FL case for LT3A II : TARIF CODE :  Ltariff_code_i == 104 
			if ((consumption / bm) < 200)
				...
				if (consumption < 280)
					...
					localtempEC += ((consumption - 400) * 4.6) + (120 * 1.82)+ (80 * 0.1);
		...
		else if(Ltariff_code_i == 53 || Ltariff_code_i == 58 || Ltariff_code_i == 63 || Ltariff_code_i ==69)
		{
			
			
			if(UtilMaster.mpresentmeterstatus.equals("2")){
				
				credite_rebate = 0;
			}else{
				credite_rebate = solarRebate(ec, fc, Ltariff_code_i, consumption, bm) ;
			}
		...
		} else if (Ltariff_code_i == 179) 
		{	// ************ 5th case**************************

			credite_rebate = (float) (ec - (consumption * 1.25));

		} else if (Ltariff_code_i == 101) 
		{	// ********************* 6th case *******************
	 ...
	 */
	public float solarRebate(float ec, float fc, int Ltariff_code_i,float consumption, float bm){
		float solar = 0 ;
		//float fcReduction =  0;
		float ecReduction =  0;
		//float fcMax =  0;
		...
		try {
			...
			FilePropertyManager.appendLog("CALCULATION ERROR : "+Log.getStackTraceString(e));
			solar = 0 ;
		}
		return solar ;
	}
	 ...
	 * @return Tax 
	 */
	public float calTax(float ec, float rateInDPoint, int Ltariff_code_i,float consumption, float bm, float fppca) 
	{
		float duty = 0;
		if (Ltariff_code_i == 2|| Ltariff_code_i == 1) 
		{
	...
	public float excessLoadPenalty(String recBMD , float sanctionedLoadKW , float meterConstant , String tariffCode)
	{   
		...
		if(excessLoad > 0)
		{
			...
			masterLogger.debug("sanctionedLoadHP---------------"+sanctionedLoadHP);
			masterLogger.debug("excessLoadPenalty--------------"+excessLoadPenalty);
			excessLoadPenalty=(float) doubRound(Math.round(sanctionedLoadHP * 35 * 2),2);
			if( "173".equalsIgnoreCase(tariffCode) ||  "174".equalsIgnoreCase(tariffCode) ||  "175".equalsIgnoreCase(tariffCode) ||  "176".equalsIgnoreCase(tariffCode))
			{
				double fc=0;
				...
				excessLoadPenalty=(float) doubRound(Math.round(excessLoad * fc * 2),2);
			}
		}
		else
		{
	 ...
	 * @return Consumption
	 */
	public long consumption(String presReading,String prevReading,float meterConstatnt)
	{   
	...
	public void setBill_Due_date(int days , Date date)
	{
		...
		// current date
		UtilMaster.setMbilledatetimestamp(new SimpleDateFormat("dd/MM/yyyyHH:mm:ss").format(cal.getTime()));
	...
	public  void mobileDateValidation(int days , String serverDate1 )
	{
		...
		{		
			...
			if ( (dayName.toUpperCase().equals("SUNDAY")) || (dayName.toUpperCase().equals("SUN")) )
				mobileDateValidation(days + 1, serverDate1);
			else
			{
				if ( (   (moblDate.after(serverDate)) || (moblDate.equals(serverDate)) )
	...
	public static String getDivisionName(int sitecode){
		...
		case 1243: {
			divionName = "R K NAGARA";
			break;
		}
		case 4330: {
			divionName = "SALIGRAMA";
			break;
		}
		case 3230: {
			divionName = "MALAVALLI-1";
			break;
		}
		case 3240: {
			divionName = "MALAVALLI-2";
			break;
		}
		case 1330: {
			divionName = "T-NARASHIPURA";
			break;
		}
		case 3242: {
			divionName = "MALAVALLI-2";
		...
		case 2310: {
			divionName = "KOLLEGALA";
			break;
		}
		case 3315: {
			divionName = "PANDAVAPURA";
			break;
		}
		case 2140: {
			divionName = "SANTHEMARALLI";
			break;
		}
		case 2110: {
			divionName = "CHNAGARA-URBAN";
			break;
		}
		case 2130: {
			divionName = "GUNDLEPET";
			break;
		}
		case 2150: {
			divionName = "BEGUR";
		...
		case 2121: {
			divionName = "HARADANHALLI";
			break;
		}
		case 3134: {
			divionName = "MANDYA RSD2";
			break;
		}
		case 4130: {
			divionName = "BELUR";
		...
		case 4210: {
			divionName = "CR PATANA";
			break;
		}
		case 4220: {
			divionName = "NUGGEHALLI";
			break;
		}
		case 4223: {
			divionName = "NUGGEHALLI";
		...
		case 1223: {
			divionName = "HOOTAGALLI";
			break;
		}
		case 1320: {
			divionName = "NANJUNGUD-RURAL";
			break;
		}
		case 1310: {
			divionName = "NANJUNGUD-URBAN";
			break;
		}
		case 3123: {
			divionName = "MANDYA RSD1";
			break;
		}
		case 4160: {
			divionName = "HASAN RURAL-2";
			break;
		}
		case 4120: {
			divionName = "HASAN RURAL-1";
			break;
		}
		case 4430: {
			divionName = "GANDASI";
		...
		case 4320: {
			divionName = "ARAKALUGUDU";
			break;
		}
		case 4340: {
			divionName = "HANGARAHALLI";
			break;
		}
		case 4350: {
			divionName = "RAMANANTHAPURA";
			break;
		}
		case 4351: {
			divionName = "RAMANANTHAPURA";
	...
	public static Logger getlogger(Context logContext , String TAG) {
		String logFilename = "log_cesc_"+  new SimpleDateFormat("yyyy_MM_dd" ).format(new Date())+".log";
		...
		try {
			...
			FilePropertyManager.appendLog("LOGGER FILE ERROE: "+Log.getStackTraceString(e));
			e.printStackTrace();
		}
		return masterLogger ;
	}
