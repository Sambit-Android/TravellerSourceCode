...
import com.pgrs.util.PGRSLogger;
...
public abstract class GenericDAOImpl<T> implements GenericDAO<T> {
	...
	public T save(final T t) {		
		...
		PGRSLogger.logger.info("saving "+t+" instance");
		try {
			session.persist(t);
			PGRSLogger.logger.info(t+" saved successful");
			session.flush();
			session.clear();
			return t;
			
		} catch (RuntimeException re) {
			PGRSLogger.logger.error("saving "+t+" failed", re);
			throw re;
		}
	}
	...
	public void delete(final Object id) {
		...
		PGRSLogger.logger.info("deleting instance");
		try {
			this.entityManager.remove(this.entityManager.getReference(type, id));
			PGRSLogger.logger.info("delete successful");
		} catch (RuntimeException re) {
			PGRSLogger.logger.error("delete failed", re);
			throw re;
		}
	}
	...
	public T update(final T t) {
		...
		PGRSLogger.logger.info("updating "+t+" instance");
		try {
			T result = this.entityManager.merge(t);
			PGRSLogger.logger.info("update successful");
			return result;
		} catch (RuntimeException re) {
			PGRSLogger.logger.error("update failed", re);
			throw re;
		}
	}
	...
	public T find(final Object id) {		
		PGRSLogger.logger.info("finding instance with id: " + id);
		try {
			return (T) this.entityManager.find(type, id);
		} catch (RuntimeException re) {
			PGRSLogger.logger.error("find failed", re);
			throw re;
		}
	}
	 ...
	 public void executeDeleteQuery(String query) {
		...
		for (int i = 0; i < entityList.size(); ++i) {
			try {
				...
				PGRSLogger.logger.info(entityList.get(i) + " saved successful");
			} catch (RuntimeException re) {
				PGRSLogger.logger.error("saving " + entityList.get(i)+ " failed", re);
				throw re;
			}
		}
