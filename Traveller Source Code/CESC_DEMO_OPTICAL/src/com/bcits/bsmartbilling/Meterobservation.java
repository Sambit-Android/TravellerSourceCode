...
import amr.bluetooth.ble.DeviceScanActivity;
...
public class Meterobservation extends Activity {
	...
	Button btn_nxt ,  btn_bacK,btnThroughRF,btnDlmsOptical,btnBluetooth;
    ...
    static final String DLMSOPTICAL="DLMSOPTICAL",
    		            RF="RF",
    		            PHOTOBILLING="PHOTOBILLING",
    		            BLUETOOTH="BLUETOOTH";
	
	public static boolean isThroughRF=false;
    public static boolean isThroughDLMS=false;
    public static boolean isThroughBluetooth=false;
	@Override
...
//		Toast.makeText(getApplicationContext(), UtilMaster.deviceInfo, Toast.LENGTH_LONG).show();
		...
		btnThroughRF=(Button)findViewById(R.id.btnThroughRF);
		btnDlmsOptical=(Button)findViewById(R.id.btnDlmsOptical);
		btnBluetooth=(Button)findViewById(R.id.btnBluetooth);
		
		
		btn_nxt.setVisibility(View.GONE);
		btnThroughRF.setVisibility(View.GONE);
		btnDlmsOptical.setVisibility(View.GONE);
	...
	protected void onStart() {
		...
		isThroughRF=false;
		isThroughDLMS=false;
		isThroughBluetooth=false;
		try {
			if(UtilMaster.getMextra5().trim() !=null && (!UtilMaster.getMextra5().trim().equals("null"))){
				edPhoneNumber.setText(UtilMaster.getMextra5().trim());
			}else {
				edPhoneNumber.setText("9731014266");
			}
		} catch (Exception e) {
		...
		btnThroughRF.setOnClickListener(new OnClickListener() {
		...
		btnDlmsOptical.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View arg0) {
				startReading(DLMSOPTICAL);
	...
	public void setConsDetails(){
		...
		String rrNoFitst2=UtilMaster.mconsumer_sc_no.substring(0, 2);
		
		if(rrNoFitst2.equals("RF")) {
			btnThroughRF.setVisibility(View.VISIBLE);
		}
		else if(rrNoFitst2.equals("DL")) {
			btnDlmsOptical.setVisibility(View.VISIBLE);
		}
		else if(rrNoFitst2.equals("BL")) {
...
///////////////// for MNR (3) and DC (4) .. present status would be same as prev status
		...
		/*By shivanand 
		 * 
		 * To block DL for the LT1 , lt2fl, lt3ol*/
	...
	private void startReading(String readingMode) {
		...
		try {
			...
			if(allowNext){
				...
				if (libraryFunction.isdataValid()) {
					...
					// Lstatus_id_c ==7
					System.out.println(" UtilMaster.getMpresentmeterstatus() : "
					...
					if(readingMode.equals(DLMSOPTICAL))
					{isThroughDLMS=true;  
						DlmsMain.readMeter(Meterobservation.this, CameraActivity.class);
					}else if(readingMode.equals(RF))
					{
					isThroughRF=true;  
					RfLibMain.readMeter(Meterobservation.this, CameraActivity.class,com.bcits.bsmartbilling.MainActivity.class,"0000BDBB","2",false); //TODO 0000BDBB
//					move_inten(Meterobservation.this, CameraActivity.class);
					}
					else if(readingMode.equals(PHOTOBILLING)) {
						move_inten(Meterobservation.this, CameraActivity.class);
					}else if(readingMode.equals(BLUETOOTH)) {
						isThroughBluetooth=true; 
						startActivity(new Intent(Meterobservation.this, DeviceScanActivity.class) );
					}
				} else {
					new AlertDialog.Builder(Meterobservation.this)
							.setTitle("Info")
							.setMessage("Mobile Date has been changed please set the Date correctly OR Download fresh data to Procced next ")
