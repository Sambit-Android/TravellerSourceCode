...
public class DatabaseHandler {
	...
	public String fetchnextconsumer(String consumerno,int type) {
		...
		try{
			...
			String querynewww = "SELECT CONSUMER_SC_NO, METERNO  FROM CONSUMER_INPUT_LIVE WHERE "+KEY_BILLEDSTATUS+" LIKE '0' ORDER BY CAST("+KEY_ID+" AS INTEGER)" ;
	...
	public String getConsumerno() {
		...
		try{
			...
			String countQuery = "SELECT CONSUMER_SC_NO FROM CONSUMER_INPUT_LIVE WHERE cast(ID as int) = (SELECT CAST(ID AS INT)+1 FROM CONSUMER_INPUT_LIVE WHERE BILLEDATETIMESTAMP IN (SELECT max(BILLEDATETIMESTAMP) FROM CONSUMER_INPUT_LIVE WHERE BILLEDSTATUS LIKE '1') ) AND BILLEDSTATUS LIKE '0'";
