...
public class FtdiManager extends Activity {
	...
	/*
	 * private void appendData(String data) //TODO { if(true ==
	 * bContentFormatHex) { if(timesMessageHexFormatWriteData < 3) {
	 * timesMessageHexFormatWriteData++; HelperClass.midToast(
	 * "The writing data won¡¦t be showed on data area while content format is hexadecimal format."
	 * ,FtdiManager.this); } return; }
	 * 
	 * if(true == bSendHexData) { data ="\nOUT > > > "+data; SpannableString
	 * text = new SpannableString(data); text.setSpan(new
	 * ForegroundColorSpan(Color.YELLOW), 0, data.length(), 0);
	 * readText.append(text); bSendHexData = false; } else {
	 * readText.append("\nIN  < < < "+data); }
	 * 
	 * int overLine = readText.getLineCount() - TEXT_MAX_LINE;
	 * 
	 * if (overLine > 0) { int IndexEndOfLine = 0; CharSequence charSequence =
	 * readText.getText();
	 * 
	 * for (int i = 0; i < overLine; i++) { do { IndexEndOfLine++; } while
	 * (IndexEndOfLine < charSequence.length() &&
	 * charSequence.charAt(IndexEndOfLine) != '\n'); }
	 * 
	 * if (IndexEndOfLine < charSequence.length()) {
	 * readText.getEditableText().delete(0, IndexEndOfLine + 1); } else {
	 * readText.setText(""); } }
	 * 
	 * }
	 */
	...
	public void connectFunction() {
		try {
			...
			if (true == ftDev.isOpen()) {
				...
				// Toast.makeText(global_context, "open device port(" + portIndex +
				// ") OK", J2xxHyperTerm.this).show();
	...
	private DeviceStatus checkDevice() {
		try {
			if (ftDev == null || false == ftDev.isOpen()) {
				...
				// HelperClass.midToast("CHECK: uart_configured == false",
				// J2xxHyperTerm.this);
			...
			try {
				...
				case UPDATE_TEXT_VIEW_CONTENT:
					if (actualNumBytes > 0) {
						...
						// appendData(String.copyValueOf(readBufferToChar, 0,
						// actualNumBytes));
