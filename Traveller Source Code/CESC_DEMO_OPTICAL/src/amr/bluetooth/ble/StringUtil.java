...
public class StringUtil
{
  ...
  */  
  ...
  public static String  setSerialDataToTextView(int disp, byte[] rbuf, int len, String sCr, String sLf , DataInterpretationPojo p) {
      int tmpbuf;
      boolean lastDataIs0x0D = false;
      StringBuilder mText = new StringBuilder();
      for (int i = 0; i < len; ++i) {
          // "\r":CR(0x0D) "\n":LF(0x0A)
          if ((p.mReadLinefeedCode == LINEFEED_CODE_CR) && (rbuf[i] == 0x0D)) {
          //	readingDemo=readingDemo+sCr;
          //	readingDemo=readingDemo+BR;
              mText.append(sCr);
              mText.append(BR);
          } else if ((p.mReadLinefeedCode == LINEFEED_CODE_LF) && (rbuf[i] == 0x0A)) {
          //	readingDemo=readingDemo+sLf;
          //	readingDemo=readingDemo+BR;
          	
              mText.append(sLf);
              mText.append(BR);
          } else if ((p.mReadLinefeedCode == LINEFEED_CODE_CRLF) && (rbuf[i] == 0x0D)
                  && (rbuf[i + 1] == 0x0A)) {
              mText.append(sCr);
              if (disp != DISP_CHAR) {
                  mText.append(" ");
          ...
          //    readingDemo=readingDemo+sLf;
           ...
           //   readingDemo=readingDemo+sLf;
              ...
              switch (disp) {
                  case DISP_CHAR:
                //  	 readingDemo=readingDemo+(char) rbuf[i];
                  ...
                  case DISP_DEC:
                   ...
                   //   readingDemo=readingDemo+String.format("%1$03d", tmpbuf);
                      mText.append(String.format("%1$03d", tmpbuf));
                      mText.append(" ");
                      break;
                  case DISP_HEX:
                //  	 readingDemo=IntToHex2((int) rbuf[i]);
  ...
  private static String IntToHex2(int Value) {
      char HEX2[] = {
