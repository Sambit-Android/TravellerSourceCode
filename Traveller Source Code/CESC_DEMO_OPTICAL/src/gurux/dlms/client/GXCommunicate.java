...
public class GXCommunicate
{
    ...
    {       
    	Utils.dirctLogcat( "Inside GXCommunicate _+_+_");
...
//        Utils.dirctLogcat("Authentication: " + auth); 
//        Utils.dirctLogcat("ClientID: 0x" + Integer.toHexString(Integer.parseInt(dlms.getClientID().toString()))); 
//        Utils.dirctLogcat("ServerID: 0x" + Integer.toHexString(Integer.parseInt(dlms.getServerID().toString()))); 
    ...
    void close() throws Exception
    {
        if (Media != null)
        {
        	Utils.dirctLogcat("DisconnectRequest");
    ...
    String now()
    {
        return new SimpleDateFormat("HH:mm:ss.SSS",Locale.getDefault()).format(java.util.Calendar.getInstance().getTime());
    }
  ...
  /*  void writeTrace(String line)
    {
        if (Trace)
        {   
           Utils.dirctLogcat(line);
        }
        PrintWriter logFile = null;
        try 
        {
        	 File dir = new File(Environment.getExternalStorageDirectory() + "");
        	  String traceFileName = dir + "/trace.txt";
            logFile = new PrintWriter(new BufferedWriter(new FileWriter(traceFileName, true)));
    ...
    { //TODO
        ...
        synchronized (Media.getSynchronous())
        {
            while (!succeeded)
            {
              //  writeTrace("<- " + now() + "\t" + GXDLMSClient.toHex(data));
                ...
                if (!succeeded)
                {
                    ...
                    if (pos++ != 3)
                    {
                    	Utils.dirctLogcat("Data send failed. Try to resend " + pos.toString() + "/3");         
                        continue;
                    }   
                    Utils.dirctLogcat("Failed to receive reply from the device in given time. Position 1"); 
                    throw new Exception("Failed to receive reply from the device in given time. Position 1");
                }
            }
            ...
            while (!dlms.isDLMSPacketComplete(p.getReply()))
            {
                ...
                if (!Media.receive(p))
                {
                	Utils.dirctLogcat("Failed to receive reply from the device in given time. Position 2");      
                	 throw new Exception("Failed to receive reply from the device in given time. Position 2");
                }
            }
        }
    ...
    {       
        ...
        if (Media instanceof GXSerial)
        {
            ...
            if (iec)
            {
               ...
               Utils.dirctLogcat("Bitrate is : " + bitrate);
                //Send ACK
        ...
        if (data.length != 0)
        {
            ...
            reply = readDLMSPacket(data);
//        	Utils.dirctLogcat("reply = readDLMSPacket(data);  ++++++++++++REPLY++++++++++\n"+GXDLMSClient.toHex(reply));
    ...
    public List<AbstractMap.SimpleEntry<GXDLMSObject, GXDLMSCaptureObject>> GetColumns(GXDLMSObject pg) throws Exception
    {
        ...
        if (code != null)
        {
        	Utils.dirctLogcat("Reading Profile Generic: " + pg.getLogicalName() + " " + code.getDescription() + " entries:" + entries.toString()); 
        }
        else
        {
           Utils.dirctLogcat("Reading Profile Generic: " + pg.getLogicalName() + " entries:" + entries.toString());
        }
