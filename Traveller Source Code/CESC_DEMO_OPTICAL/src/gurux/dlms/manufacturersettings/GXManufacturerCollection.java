...
public class GXManufacturerCollection extends java.util.ArrayList<GXManufacturer>
{
    ...
    */
    ...
    {        
        ...
        DateFormat tmFormater = new SimpleDateFormat("MM-dd-yyyy",Locale.ENGLISH);        
        try
        {
    ...
    */
    ...
    public static void readManufacturerSettings(GXManufacturerCollection Manufacturers, String path,Activity activity)
    {
        Manufacturers.clear();
        ...
        if (files != null)
        {
            ...
            for (String it : files)
            {
                ...
                if (it.endsWith(".obx"))
                {
                    ...
                    Manufacturers.add(parse(in,activity));
                    }
                    catch(Exception ex)
                    {
                    	ex.printStackTrace();
                        System.out.println(ex.getMessage()+" .obx read error");
                        continue;
                    }
                }
            }
        	...
        	System.out.println("No files found or no such directory +++++++++");
        }
    }
     ...
     * @throws Exception
     */
    private static GXManufacturer parse(InputStream in,Activity activity) throws Exception
    {
    	
    	//TODO Parsing xml file
        ...
        final XmlPullParser xmlStreamReader = inputFactory.newPullParser();
      
       // System.out.println(")_)_)_)_) Inside parse");
        
        xmlStreamReader.setInput(in,null);
       ...
       // final TextView txt= (TextView)activity.findViewById(R.id.txtProgressInfo);
        ...
        { 
            ...
            {
            	//System.out.println("START_DOCUMENT   "+xmlStreamReader.getName());
                attributes.clear();
                data = null;
                target = xmlStreamReader.getName();
                ...
                {
                    attributes.putValue(xmlStreamReader.getAttributeName(i), xmlStreamReader.getAttributeValue(i));
                }
            }
               ...
               /* activity.runOnUiThread(new Runnable() 
                {
                     public void run() 
                     {
                    	  ...
                    	  txt.setText("READING MANUFACTURER\n"+xmlStreamReader.getText());
                     }
            ...
            else if(event==XmlPullParser.END_TAG)
            {
                ...
                {
                    man.setInactivityMode(gurux.dlms.manufacturersettings.InactivityMode.valueOf(data.toUpperCase(Locale.ENGLISH)));
                }
                else if(target.equalsIgnoreCase("Type"))
                {
                    if (authentication != null)
                    {
                        authentication.setType(gurux.dlms.enums.Authentication.valueOf(data.toUpperCase(Locale.ENGLISH)));
                    }
                    else
                    {
                        ...
                        {
                            att.setType(gurux.dlms.enums.DataType.valueOf(data.toUpperCase(Locale.ENGLISH)));
                        }
                    }
                }
                 else if(target.equalsIgnoreCase("UIType"))
                {
                    ...
                    {
                        att.setUIType(gurux.dlms.enums.DataType.valueOf(data.toUpperCase(Locale.ENGLISH)));
                    }
                }
                else if(target.equalsIgnoreCase("GXAuthentication"))
                {
                ...
                {
                    man.setStartProtocol(StartProtocolType.valueOf(data.toUpperCase(Locale.ENGLISH)));
                }                
                else
                {
                    target = "";
                ...
                target = "";
              //  System.out.println("=====END_TAG   "+xmlStreamReader.getName());
                if(xmlStreamReader.getName().equals("GXManufacturer")){
                	return man;
                }
            }

        }
        System.out.println("return man ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
        return man;
    }
}