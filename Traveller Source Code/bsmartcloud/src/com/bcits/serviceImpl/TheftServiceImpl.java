...
import com.bcits.service.TheftService;
import com.bcits.utility.BCITSLogger;

@Repository
public class TheftServiceImpl extends GenericServiceImpl<TheftRecordingEntity> implements TheftService
{
	...
	{
		List<TheftRecordingEntity> theftList=findAll();
		model.put("theftList", theftList);
		List<String> statusList=getCheckConstraints("THEFT", "STATUS_CHECK",request);
		model.put("statusList", statusList);
	}
	@Override
	@Transactional(propagation=Propagation.SUPPORTS)
	public List<TheftRecordingEntity> findAll() 
	{
		return entityManager.createNamedQuery("TheftRecordingEntity.findAll").getResultList();
	}
	...
	public byte[] findOnlyImage(ModelMap model,HttpServletRequest request,HttpServletResponse response,String consumer_Sc_No,String value) throws IOException 
	{
		int k=0;
		int t=0;
		List<TheftRecordingEntity> hh=  entityManager.createNamedQuery("TheftRecordingEntity.findOnlyImage").setParameter("consNo",consumer_Sc_No).getResultList();
	...
	{
		Query query=entityManager.createNamedQuery("TheftRecordingEntity.updateStatus").setParameter("id", id).setParameter("status", status);
