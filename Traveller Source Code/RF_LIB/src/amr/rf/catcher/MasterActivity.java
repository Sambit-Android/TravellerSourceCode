...
import amr.rf.utils.FtdiManager;
...
public class MasterActivity extends FtdiManager {
	...
	LinearLayout linearReadingFrame,linearSearchingFrame,linearSearchingFrameMode;
	Button btnReadMeter,btnDisconnect,btnReconnect,btnDiscover,btnStopDiscover;
	
	public static Context context;
	IntentFilter filter = new IntentFilter();
	public static String logNameTime; 
    public static boolean isReadingMeter; //CONROLLING ALL THE THREADS AND OTHER ACTIVITIES
    ...
    public static final String GET_RELAY_STATUS = "GET RELAY STATUS";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		linearSearchingFrameMode=(LinearLayout)findViewById(R.id.linearSearchingFrameMode);
...
//		linearReadingFrame.setVisibility(View.GONE);
	...
	public void onBackPressed() {
		...
		try {
			RfLibMain.isGroupBilling=false;
			startActivity(new Intent(MasterActivity.this,  RfLibMain.activityIn));
			MasterActivity.this.finish();
		} catch (Exception e) {
			MasterActivity.this.finish();
...
//				linearReadingFrame.setVisibility(View.GONE);
						...
						if(SUCCESS.equals(response))
 {
						...
						if (SUCCESS.equals(response)) { // MAIN READING HAPPENS HERE TODO

							if (readingType.equals(BILLING)) {
								writeToDevice(FrameMaster.readInstantaneous(SOURCE_ADDRESS, METER_ADDRESS));
								response = parseResponse(BILLING, false);
								publishProgress(BILLING + " " + response);
							}
							else if (readingType.equals(DISCONNECT_METER) || readingType.equals(RECONNECT_METER)) { // DISCONNECT OR RECONNECT
								writeToDevice(FrameMaster.disConnectRelayStatus(SOURCE_ADDRESS,	METER_ADDRESS)); // CHECKING CURRENT RELAY STATUS
								response = parseResponse(GET_RELAY_STATUS,false);
								publishProgress(GET_RELAY_STATUS + " "+ response);
								if(SUCCESS.equals(response)) {
								if (readingType.equals(RECONNECT_METER)) {
									if (finalResponse.equals(RECONNECTED)) {
										finalResponse = "ALREADY "+ RECONNECTED;
									} else {
										writeToDevice(FrameMaster.reConnectMeter(SOURCE_ADDRESS,METER_ADDRESS));
										response = parseResponse(RECONNECT_METER, false);
										publishProgress(RECONNECT_METER + " "+ response);
									}
								} else if (readingType.equals(DISCONNECT_METER)) {
									if (finalResponse.equals(DISCONNECTED)) {
										finalResponse = "ALREADY "+ DISCONNECTED;
									} else {
										writeToDevice(FrameMaster.disConnectMeter(SOURCE_ADDRESS,METER_ADDRESS));
										response = parseResponse(DISCONNECT_METER, false);
								...
								writeToDevice(FrameMaster.closeChannel(	SOURCE_ADDRESS, METER_ADDRESS)); // CLOSING OPENED CHANNEL
...
//				linearReadingFrame.setVisibility(View.VISIBLE);
	...
	public synchronized String parseResponse(String readingType,boolean isGroupBilling) { 
		try {
				...
				if (null != receivedData)
					...
					if (null!=dataInHex && !dataInHex.isEmpty()) 
						if (dataInHex.length() > DATA_MIN_LENGTH && dataInHex.contains(MAGIC_NUMBER)) // CHECK MINIMUM LENGTH TO GET THE SIZE OF FRAME
							...
							case OPEN_CHANNEL:
								System.out.println(OPEN_CHANNEL+"  frameId  "+frameId);
								if(frameId.equals("80")) {
									return SUCCESS;
								}else {
								return	frameException(readingType,frameId);
							...
							case BILLING:
								if(frameId.equals("80")) {
									 ...
									 if(isGroupBilling) { // IF IT IS GROUP BILLING, RETURN KWH VALUE TO THE GROUP BILLING THREAD
										 return finalResponse+SPLITTER+SUCCESS;
									 }
									 return SUCCESS;
								}else {
							...
							case GET_RELAY_STATUS:
								if(frameId.equals("80")) {
									finalResponse=isDiconnected(dataInHex,GET_RELAY_STATUS);
									return SUCCESS;
								}else {
									return	frameException(readingType,frameId);
								} 
...
/*	public String isDiconnected(String dataInHex,String readingType) {
	...
	*/
	public String isDiconnected(String dataInHex,String readingType) {
		FileManager.writeLog("DISCONNECT FULL4 DATA   ==  "+ dataInHex);
		
		 String dataAfterMagicNo = dataInHex.split(MAGIC_NUMBER)[2].trim(); // TAKING SECOND MAGIC NUMBER FOR BILLING DATA
		 String afterStartFrame=dataAfterMagicNo.split("C4E1")[1].trim();
		 String data=afterStartFrame.substring(4,6);
		 FileManager.writeLog("DISCONNECT DATA   ==  "+ data);
		 if((readingType.equals(DISCONNECT_METER)||readingType.equals(GET_RELAY_STATUS)) && data.equals("00")) {
			 return DISCONNECTED;
		 }else if((readingType.equals(RECONNECT_METER)||readingType.equals(GET_RELAY_STATUS)) && data.equals("01")){
			 return RECONNECTED;
		 }
		 else if(readingType.equals(GET_RELAY_STATUS) && data.equalsIgnoreCase("FF")){
		 return "RELAY STATE NOT SUPPORTED";
		 }
		 else if(readingType.equals(GET_RELAY_STATUS) && data.equalsIgnoreCase("15")){
			 return "ERROR FAILED COMMAND";
			 }
		return "FAILED";
	}
	...
	if(readingType.equals(BILLING)|| readingType.equals(DISCONNECT_METER) || readingType.equals(RECONNECT_METER)|| readingType.equals(GET_RELAY_STATUS))
	{
