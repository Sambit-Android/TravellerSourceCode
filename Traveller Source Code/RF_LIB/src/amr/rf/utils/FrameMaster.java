...
public class FrameMaster {
    ...
    public static byte[] ping() { //23 means 17
        ...
        System.out.println("PING  " + bytesToHex(pingArray));
        return pingArray;
    }
    
    public static byte[] discover(String deviceAddress) {
    	FileManager.writeLog("DISCOVER METER >>>");
    ...
    public static byte[] setContext(String deviceAddress, int meterChannel) { // SETTING THE COMMUNICATION CHANNEL
    	FileManager.writeLog("SET CONTEXT ON CHANNEL "+meterChannel+">>>");
          ...
          System.out.println("SET CONTEXT  " + bytesToHex(setContext));
          return setContext;
    }
    
    public static byte[] openChannel(String deviceAddress,String meterAddress) {
    	FileManager.writeLog("OPEN CHANNEL >>>");
    	 byte[] openChannel = createByteFrame( 1, hexStringToByteArray(deviceAddress), hexStringToByteArray(meterAddress), new byte[0]);
         System.out.println("OPEN CHANNEL  " + bytesToHex(openChannel));
         return openChannel;
    }
    public static byte[] closeChannel(String deviceAddress,String meterAddress) {
    	FileManager.writeLog("CLOSE CHANNEL >>>");
    	byte[] closeChannel = createByteFrame( 2, hexStringToByteArray(deviceAddress), hexStringToByteArray(meterAddress), new byte[0]);
        System.out.println("CLOSE CHANNEL  " + bytesToHex(closeChannel));
        return closeChannel;
   }
    
    public static byte[] disConnectRelayStatus(String deviceAddress,String meterAddress) { //c4e1018c7e3a
    	FileManager.writeLog("GET RELAY STATUS>>>");
   	 	byte[] relayStatus = createByteFrame( 3, hexStringToByteArray(deviceAddress), hexStringToByteArray(meterAddress),  hexStringToByteArray("c4e1018c7e3a"));
   	 	System.out.println("GET RELAY STATUS  " + bytesToHex(relayStatus));
   	 	return relayStatus;
   }
    
    public static byte[] disConnectMeter(String deviceAddress,String meterAddress) { //c4e1028e009a6b
    	FileManager.writeLog("DICONNECT METER >>>");
   	 	...
   	 	System.out.println("DICONNECT METER  " + bytesToHex(diconnnect));
   	 	return diconnnect;
   }
    
    public static byte[] reConnectMeter(String deviceAddress,String meterAddress) { //c4e1028e018a4a
    	FileManager.writeLog("RECONNECT METER >>>");
    ...
    public static byte[] readInstantaneous(String deviceAddress,String meterAddress) { //c4e101604298
    	FileManager.writeLog("READ METER >>>");
   	 	...
   	 	System.out.println("READ METER  " + bytesToHex(openChannel));
   	 	return openChannel;
   }
    ...
    public static byte[] createByteFrame(int frameId, byte[] source, byte[] destination, byte[] data) {
        ...
        System.out.println("hexStringToByteArray ========== "+hexStringToByteArray.length);

        if(hexStringToByteArray.length==1) { // IF CRC COMING AS 8 bit make it as 16 bit
        	 byte [] dummy= {(byte) 0,hexStringToByteArray[0]};
        	 hexStringToByteArray=dummy;
        	 System.err.println("hexStringToByteArray ==================== "+hexStringToByteArray.length);
        }
