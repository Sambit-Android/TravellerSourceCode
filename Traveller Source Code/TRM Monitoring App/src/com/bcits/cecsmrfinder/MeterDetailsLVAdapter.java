...
public class MeterDetailsLVAdapter extends BaseAdapter {
	...
	public MeterDetailsLVAdapter(Context contxt) throws JSONException {
		// TODO Auto-generated constructor stub
		
		context = contxt;
		arr = new JSONArray(ConsumerHistoryAllDetails.responsefromservermeterdetails);
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		rowView = inflater.inflate(R.layout.consumer_history_meter_details_singleitem, null);
		...
		try {
			...
			String udata=arr.getJSONObject(position).getString("FIXEDDATE");
			 SpannableString content = new SpannableString(udata);
			 content.setSpan(new UnderlineSpan(), 0, udata.length(), 0);
			
			holder.billmonth.setText(content);
			holder.billunit.setText(arr.getJSONObject(position).getString("SERIALNO"));
			holder.status.setText(arr.getJSONObject(position).getString("MAKE"));
			holder.netamt.setText(arr.getJSONObject(position).getString("CAPACITY"));
