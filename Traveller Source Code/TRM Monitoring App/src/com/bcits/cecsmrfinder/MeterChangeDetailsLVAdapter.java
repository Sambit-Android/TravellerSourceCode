...
public class MeterChangeDetailsLVAdapter extends BaseAdapter {
	...
	public MeterChangeDetailsLVAdapter(Context contxt) throws JSONException {
		// TODO Auto-generated constructor stub
		
		context = contxt;
		arr = new JSONArray(ConsumerHistoryAllDetails.responsefromservermeterchangedetails);
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		rowView = inflater.inflate(R.layout.consumer_history_meterchange_details_singleitem, null);
		...
		try {
			...
			String udata=arr.getJSONObject(position).getString("MCDATE");
			 SpannableString content = new SpannableString(udata);
			 content.setSpan(new UnderlineSpan(), 0, udata.length(), 0);
			
			holder.billmonth.setText(content);
			holder.billunit.setText(arr.getJSONObject(position).getString("OLDSERIALNO"));
			holder.status.setText(arr.getJSONObject(position).getString("NEWMAKE"));
			holder.netamt.setText(arr.getJSONObject(position).getString("NEWSERIALNO"));
