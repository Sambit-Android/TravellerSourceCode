...
public class BillingDetailsLVAdapter extends BaseAdapter {
	...
	public BillingDetailsLVAdapter(Context contxt) throws JSONException {
		// TODO Auto-generated constructor stub
		
		context = contxt;
		
		
			arr = new JSONArray(ConsumerHistoryAllDetails.responsefromserverbillingdetails);
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		rowView = inflater.inflate(R.layout.consumer_history_billing_details_singleitem, null);
			...
			try {
				...
				String udata=arr.getJSONObject(position).getString("MONTHYEAR");
				 SpannableString content = new SpannableString(udata);
				 content.setSpan(new UnderlineSpan(), 0, udata.length(), 0);
				
				
				holder.billmonth.setText(content);
				holder.billunit.setText(arr.getJSONObject(position).getString("UNITS"));
				holder.status.setText(arr.getJSONObject(position).getString("STATUS"));
				holder.netamt.setText(arr.getJSONObject(position).getString("NET"));
