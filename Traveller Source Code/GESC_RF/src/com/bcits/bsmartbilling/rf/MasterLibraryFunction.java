...
public class MasterLibraryFunction extends Application {
	...
	public int calculate() throws Exception {
		...
		try {
			...
			*/
			...
			/*// dated 06/04/2016 fppca for flat rates
			others = (float) (units * 0.04);*/
			

			
			// dated 01/07/2016 fppca for flat rates
			...
			*/
			...
			// New 
			if ((UtilMaster.mextra3.trim().equals("Y")) || ((UtilMaster.mtariff.trim().equalsIgnoreCase("2")) && (((units / bm) <= 18)))) {
				tax = 0.0f;
			}
	...
	public float getECForAllQuarters(Context mobContext, float nou, float bm,
		...
		/*	for (int cnt = 0; cnt < tdates.length; cnt++) {
				...
				difBM = dateDiff("m",
						new SimpleDateFormat("dd/MM/yyyy").parse(UtilMaster.mprevious_billeddate), getMobileDate(new SimpleDateFormat(
	...
	public float calcFCForLT7TariffOnly(float days) {
		...
		try {
			...
			ratePerDay = (Float.valueOf(UtilMaster.mconnected_load.trim()) * 170) / 7;
