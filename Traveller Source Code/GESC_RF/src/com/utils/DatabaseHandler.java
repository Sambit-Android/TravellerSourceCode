...
import com.bcits.bsmartbilling.rf.UtilMaster;
...
public class DatabaseHandler {
	...
	private static String MYDATABASE_NAME = dir + "/GESCOM/GULBARGAELECTRICITYBOARD.db";
	...
	public void insert(String KEY_CONSUMER_SC_NO1 ,
		...
		if (db_status) {
			...
			if (mCursor != null && mCursor.moveToFirst()) {
				...
				contentValues.put( KEY_EXTRA1 , "0" );
				contentValues.put( KEY_EXTRA2 , "0" );

				/*extra3 for taxflag, extra4 for receipt date*/

				contentValues.put( KEY_EXTRA3,  KEY_EXTRA11 );
				contentValues.put( KEY_EXTRA4  , KEY_EXTRA21 );


				contentValues.put( KEY_EXTRA5 , cmobileNumber);
				contentValues.put( KEY_EXTRA6 , key_extra6 );
				contentValues.put( KEY_EXTRA7 , reciptAmount );
	...
	public int Updatetolocaldb(String meterconstant, String presentreading,
		...
		System.out.println("Present Meter status is :"+UtilMaster.mpresentmeterstatus);
		...
		try{
			...
			if (cursor.moveToFirst()) {
				do {
					...
					/*String bank = cursor.getString(cursor.getColumnIndex(KEY_FPPCA))+",GESCOM_VM"+version_name+",0,0,0,"+cursor.getString(cursor.getColumnIndex(KEY_EXTRA5));*/

					String bank = onlyFppca+",GESCOM_VM"+version_name+",0,0,0,"+cursor.getString(cursor.getColumnIndex(KEY_EXTRA5));
	...
	public String getServerToSBMDate() throws SQLException {
		...
		String q = "SELECT  "+KEY_SERVERTOSBMDATE+" FROM  " + MYDATABASE_TABLE + " LIMIT 1 ; ";
	...
	public String fetchnextconsumer(String consumerno,int type) {
		...
		try{
			/*cursor = sqLiteDatabase.query(MYDATABASE_TABLE, new String[] { KEY_CONSUMER_SC_NO,KEY_METERNO}, null, null, null, null, null, "CAST(FOLIO_NO AS INTEGER)");*/


			String querynewww = "SELECT CONSUMER_SC_NO, METERNO  FROM CONSUMER_INPUT_LIVE WHERE "+KEY_BILLEDSTATUS+" LIKE '0' ORDER BY CAST("+KEY_FOLIO_NO+" AS INTEGER)" ;

			cursor = sqLiteDatabase.rawQuery(querynewww, null);
	...
	public String getConsumerno() {
		...
		/*boolean flagcheck = false;
          ...
          try{
        	  ...
        	  String firstQuerry = "SELECT max(BILLEDATETIMESTAMP) FROM CONSUMER_INPUT_LIVE";





			cursor = sqLiteDatabase.rawQuery(firstQuerry, null);
			if(cursor.moveToFirst()){

				flagcheck = true;

				System.out.println("Coming and flagcheck is true");
		...
		try{
			...
			System.out.println("Executing the first block");

			String countQuery = "SELECT CONSUMER_SC_NO FROM CONSUMER_INPUT_LIVE WHERE cast(FOLIO_NO as int) = (SELECT CAST(FOLIO_NO AS INT)+1 FROM CONSUMER_INPUT_LIVE WHERE BILLEDATETIMESTAMP IN (SELECT max(BILLEDATETIMESTAMP) FROM CONSUMER_INPUT_LIVE WHERE BILLEDSTATUS LIKE '1') ) AND BILLEDSTATUS LIKE '0'";
		...
		if(consumerno == null || consumerno.trim().equals("")){


			try{


				System.out.println("Executing the second block");
	 ...
	 * @return CONSUMER NO
	 */
	public String[] getlastStoredLatLong() {
		...
		Cursor cursor = null;
		try{
			String countQuery = "SELECT "+KEY_LONGITUDE+","+KEY_LATTITUDE+" FROM CONSUMER_INPUT_LIVE   WHERE  BILLEDATETIMESTAMP = (SELECT MAX(BILLEDATETIMESTAMP)  FROM CONSUMER_INPUT_LIVE);";
			...
			System.out.println("Lattitude is :"+a[0]);
			System.out.println("Longitude is :"+a[1]);
